...
enum class AntiWindup {
    BACK_CALCULATION,      // Traditional dt-based back-calculation
    CONDITIONAL_INTEGRATION // Ki-based conditional integration
};
...
public:
    /**
     * @brief Construct a new Float Controller object
     * 
     * @param kp Proportional gain
     * @param ki Integral gain
     * @param kd Derivative gain
     * @param sample_time Sample time in seconds
     * @param gamma Back-calculation factor for integral windup prevention
     * @param enable_back_calculation Enable back-calculation for integral windup prevention
     */
    ...
    T update(T setpoint, T measured_value, T dt) {
        ...
        // Store old integral for back-calculation
        T old_integral = integral_;
        ...
        if (enable_back_calculation_ && output != unclamped_output) {
            T windup_error = unclamped_output - output;
            
            if (antiwindup_method_ == AntiWindup::BACK_CALCULATION) {
                // Traditional time-based back-calculation (your original method)
                integral_ -= windup_error * dt / gamma_;
            } else {
                // Conditional integration method (corrected)
                // Only adjust the integral by the amount that caused windup
                if (ki_ != 0) {
                    T integral_correction = windup_error / ki_;
                    integral_ -= integral_correction / gamma_;
                }
            }
        }
    ...
    void setAntiWindupMethod(AntiWindup method) { antiwindup_method_ = method; }
    AntiWindup getAntiWindupMethod() const { return antiwindup_method_; }
...
private:
    ...
    AntiWindup antiwindup_method_ = AntiWindup::BACK_CALCULATION; // Default to traditional method
